...
def select_mode(key, mode):
    ...
    # Handle key combinations starting with Q, W, E, R, T
    if key == ord('q') or key == ord('w') or key == ord('e') or key == ord('r') or key == ord('t'):  
        next_key = cv.waitKey(10)
        while next_key != -1 and (48 <= next_key <= 57):  # Continue checking for next key while it's valid
            number = (key - ord('q')) * 10 + (next_key - 48)
            next_key = cv.waitKey(10)  # Check next key
        if next_key != -1:  # If the next key is not part of the combination, put it back to the event queue
            cv.waitKey(0)
